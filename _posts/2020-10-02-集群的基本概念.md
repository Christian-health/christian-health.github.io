...
务，也能够工作在HA的场景当中，也能够提供HA的功能，`CRM`是一个通用层。
...
主节点`ping`不通了，那么就说明是主节点挂了，就可以隔离对方。除了这种方法之外我们还可以使用仲裁磁盘的机制。主节点不
...
每个节点的票数，可以理解为权重可以是不通的，`A`和`B`的票数高于`C`和`D`的。那么`A`和`B`就可以组成一个集群继续工作。
...
(4) keepalived
组织机制和上面的三个不同。
在vip基于vrp机制实现的，可以简单的配置成为双主模型。keepalived是lvs专门开发的。
...
```
三个配置文件：
1、密钥文件，权限是600, 这个文件通常被成为authkeys
...
HA services:
一个高可用服务可以包含多个资源。从属于一个服务的多个资源应该运行在同一个节点上，如果不定义成一个组的话，这些资源在启动的时候模式是平衡的法则，一个集群定义多个资源，这些资源是平均运行在各个节点上的，所以你有两个资源，两个节点上，那么一定是一个节点上运行了一个资源。比如Web服务需要vip, httpd, filesystem三个资源，那么如果没有归为一组的话，就可能是两个资源运行在一个节点上，另外的一个资源运行在另一个节点上。把他们作成一个组，这个组的要么运行在A节点上，要么运行在B节点上。所以我们需要他们同进同退的时候就需要group，资源同级同退。
...
Resource Type（资源类型）:
    ...
    (3)clone 克隆资源，运行在每一个节点上
		STONITH
		Cluster Filesystem 集群文件系统，集群文件系统运行起来的时候意味着，可以同时写读同一个共享存储的。意味着一个节点写文件的时候一定会加锁的。这个锁持有的信息一定要通知给其他其他能够读写同一个文件的节点。就是依靠dlm: Distributed Lock Manager分布式锁管理器。运行在某一个节点上的锁管理器，一旦持有了某个文件的锁。一定要将这个信息通给其他的节点。让其他的节点都能看到。所以像这种资源它也要运行在某一个节点上的。
...
&emsp;`corosync` 本身就是一个集群引擎，只能进行集群事务信息传递的，不具备资源管理能力，由此想在`corosync`基础之上，让那
