...
（3） 派生一个专用**连接线程**（接收SQL，返回结果），多一个会话就会多一个出来线程，多一个查询结果。  
...
```
（2）验证语义 =》到底是 DQL ，DDL，DML ，DCL  中的哪一种？   
（3）验证权限   
...
根据SQL层提供的**取数据的方法**，拿到数据，返回给SQL层，**结构化成表**，再又连接层线程返回给用户。  
...
新加一个磁盘，分区，格式化，挂载，然后使用。分区，格式化在底层做了一件什么事情那？
...
https://blog.csdn.net/menghuangxiao/article/details/82898351?utm_medium=distribute.pc_relevant_download.none-task-blog-baidujs-1.nonecase&depth_1-utm_source=distribute.pc_relevant_download.none-task-blog-baidujs-1.nonecase
