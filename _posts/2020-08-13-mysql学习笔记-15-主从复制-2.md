...
master_user='repl',
master_password='123' ,
MASTER_AUTO_POSITION=1; 自动进行一个pos号的应用，我们之前是必须要自定明确的起点，但是有了GTID后就不需要了，你就设置1，代表
start slave;
```

###### 2.3.4  GTID 复制和普通复制的区别
（0）在主从复制环境中，主库发生过的事务，在全局都是由唯一`GTID`记录的，更方便`Failover`（Failover 故障转移; 故障切换; 容错; 失效接管; 失效转移 ）
（1）额外功能参数（3个）
（2）`change master to` 的时候不再需要`binlog` 文件名和`position`号`,MASTER_AUTO_POSITION=1`;
（3）在复制过程中，从库不再依赖`master.info`文件，而是直接读取最后一个`relaylog`的 `GTID`号
