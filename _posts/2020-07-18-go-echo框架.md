...
```
```
如果参数是这样的 curl -g -X GET 'http://localhost:1323/getIDs?ids=["e0cf534a-0890-4f93-aa3a-17a180477dad","f84ddae5-d7ac-467f-bb17-7216f3c412bc","8bba8ddb-3fe7-431a-b922-2680808d57df","ac9b81cb-6ec9-4226-9e5f-d1f28985c838"]'
...
//#这里非常重要，这里完成了一个go的 string转换成为[]string的过程
//具体参考了：https://stackoverflow.com/questions/59556504/evaluate-string-in-golang-string-to-string
func queryIDs(c echo.Context) error {
	names := c.QueryParam("ids")
	ids := []string{}
	err := json.Unmarshal([]byte(names), &ids)
	if err == nil {
		fmt.Printf("%v\n", ids)
	}
	for k, v := range ids {
...
/*curl -v -X   GET "http://127.0.0.1:1323/login" -H "Content-Type: application/json" -H"Authorization: Basic YWxhZGRpbjpvcGVuc2VzYW1l"
这个curl命令见参考文献15，以前做别的任务时候使用过一次
[root]$ go run main.go
...
*/
//使用 BasicAuth 中间件，Basic YWxhZGRpbjpvcGVuc2VzYW1l解密过程就不需要我们来做了，直接交给 BasicAuth 中间件就好了
func auth(username, password string, c echo.Context) (bool, error) {
	fmt.Println("username", username) //username: aladdin
	fmt.Println("password", password) //password: opensesame
	return true, nil
}
...
func main() {
	...
	e.Use(middleware.BasicAuthWithConfig(middleware.BasicAuthConfig{
		/* Skipper 定义了一个跳过中间件的函数,如果这个函数返回true那么就不执行下面的customAuthValidator，
		其实就是跳过了这个中间件函数*/
...
func customSkipper(echo.Context) bool {
	fmt.Println("customSkipper")
	return false
}

func customAuthValidator(userName, passWord string, c echo.Context) (result bool, err error) {
	fmt.Println("customAuthValidator")
	result = true
	err = nil
	return
}
...
func main() {
	...
	e.GET("/", func(c echo.Context) error {
		...
		c.Response().WriteHeader(http.StatusOK)		
		if err := json.NewEncoder(c.Response()).Encode(locations); err != nil {
			return err
		}
		return nil
	})
...
{"Altitude":42,"Latitude":33.812092,"Longitude":-117.918974}
{"Altitude":15,"Latitude":37.77493,"Longitude":-122.419416}
][root@LIN-6A14041F8B1 libs]# 

```
```go
//
package main
//http请求返回json数组，如何解析json数组
import (
	"encoding/json"
	"fmt"
	"strings"
...
func main() {
	...
	if err := json.NewDecoder(reader).Decode(&userSlice); err != nil {
		fmt.Printf("%v", err)
	} else {
...
func main() {
	...
	fmt.Println("================================================")
	somethingPoint := doSomethingPoint()
	fmt.Println(somethingPoint)
	fmt.Println(*somethingPoint)
	for _, thing := range *somethingPoint {
		fmt.Println(thing.Name)
	}
}
...
func doSomethingPoint() *[]SomeStruct {
	ServicesSlice := make([]SomeStruct, 0, 2)
	ServicesSlice = append(ServicesSlice, SomeStruct{"somename1", "someurl1", 200})
	ServicesSlice = append(ServicesSlice, SomeStruct{"somename2", "someurl2", 500})
	return &ServicesSlice
...
————————————————
版权声明：本文为CSDN博主「Dev-L」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/u010525694/article/details/89052608
```
...
- [14、Golang：BasicAuth + JWT 校验用户权限写的详细](https://juejin.im/post/5e65c4b451882549575f9dde#heading-2)
- [15、HTTP/Headers/Authorization](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Authorization)
- [16、go context经典介绍](https://www.flysnow.org/2017/05/12/go-in-action-go-context.html)
- [17、uml图](http://plantuml.com/zh/guide)
