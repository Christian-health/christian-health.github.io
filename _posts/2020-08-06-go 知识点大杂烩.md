...
```
Caller报告当前go程序调用栈所执行的函数的文件和行号信息。实参skip为上溯的**栈帧数**，0表示Caller的调用者（Caller所在的调用栈）。（由于历史原因，skip的意思在Caller和Callers中并不相同。）函数的返回值为调用栈标识符、文件名、该调用在文件中的行号。如果无法获得信息，ok会被设为false。pc是函数指针。
```
type Func
type Func struct {
    // 内含隐藏或非导出字段
